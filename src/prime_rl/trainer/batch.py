...
def prepare_sample(training_example: TrainingSample, seq_len: int) -> MicroBatch:
    ...
    # (e.g. via a-posteriori step-judge credit assignment), use them verbatim
    # for the completion span. Prompt tokens always get the scalar advantage
    # (they are loss-masked anyway, so the value is cosmetic but must match
    # `input_ids` len).
